...
export const updateTask = handleAsyncError(async function (
    req: Request,
    res: Response,
    next: NextFunction,
) {
    const body = await updateTaskSchema.parseAsync(req.body);
    const workspaceId = await workspaceIdSchema.parseAsync(req.params["workspaceId"]);
    const projectId = await projectIdSchema.parseAsync(req.params["projectId"]);
    const taskId = await taskIdSchema.parseAsync(req.params["taskId"]);
    ...
    roleGuard({
        role: membership.role as RoleDocument,
        requiredPermissions: [PermissionEnum.EDIT_TASK],
    });
